...
/**
 * Maps an OpenCode error name to a runtime error class used by the
...
/**
 * Returns a human-readable label for the OpenCode error name.
 */
...
/**
 * Returns whether an OpenCode error is retryable, if the information is
...
/**
 * Extracts a human-readable error message from an OpenCode `session.error`
 * event, combining the error label with any detail from the payload.
 *
 * Each OpenCode error type has a specific `data` shape (from the SDK):
