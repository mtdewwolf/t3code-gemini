...
import type {
  ...
  OpenCodeConfiguredProvider,
  OpenCodeDiscoveredModel,
  OpenCodeFileDiff,
  OpenCodeListedProvider,
  OpenCodeModel,
  OpenCodeProviderSession,
  OpenCodeTodo,
  OpenCodeToolState,
...
export function stripTransientSessionFields(session: OpenCodeProviderSession) {
...
export function parseOpencodeModel(model: string | undefined):
  | {
      providerId: string;
      modelId: string;
      variant?: string;
...
export function toOpencodeRequestType(permission: string | undefined): CanonicalRequestType {
  switch (permission) {
    case "bash":
...
export function toolStateTitle(state: OpenCodeToolState): string | undefined {
  switch (state.status) {
    case "pending":
      return undefined;
    case "running":
...
export function toolStateDetail(state: OpenCodeToolState): string | undefined {
  switch (state.status) {
    case "pending":
      return undefined;
    case "running":
...
/**
 * Converts an array of OpenCode file diffs into a single unified diff string.
 * The format approximates standard unified diff output (--- a/file, +++ b/file,
 * with addition/deletion counts) without full line-level hunks since OpenCode
 * only provides before/after snapshots and summary counts.
 */
export function fileDiffsToUnifiedDiff(diffs: ReadonlyArray<OpenCodeFileDiff>): string {
  if (diffs.length === 0) {
    return "";
  }
  return diffs
 ...
 */
export function todoPriorityPrefix(todo: OpenCodeTodo): string {
